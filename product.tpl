				...
				<div id="views_block" class="clearfix {if isset($images) && count($images) < 2}hidden{/if}">
							...
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
										<img class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'medium_default')|escape:'html':'UTF-8'}" alt="{$imageTitle}" title="{$imageTitle}"{if isset($mediumSize)} height="{$mediumSize.height}" width="{$mediumSize.width}"{/if} itemprop="image" />
									</a>
								</li>
							{/foreach}
						{/if}
					...
					{if isset($images) && count($images) > 2}
						<a id="view_scroll_right" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">
							<i class="icon-angle-right"></i>{l s='Next'}
						</a>
					{/if}
				</div> <!-- end views-block -->
				<!-- end thumbnails -->
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					...
					<div class="box-cart-bottom">
						...
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if} class="ttaddtocart">
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
		...
		<section class="tt-tabcontent">
			<ul id="producttab" class="nav nav-tabs clearfix">
				{if isset($product) && $product->description}
					<li class="active">
					<a href="#ttmoreinfo" data-toggle="tab" class="producttab-heading">{l s='More info'}</a>
					</li>
				{/if}
				{if isset($features) && $features}
					<li class="">
					<a href="#ttdatasheet" data-toggle="tab" class="producttab-heading">{l s='Data sheet'}</a>
					</li>
				{/if}
				{if isset($attachments) && $attachments}
					<li class="">
					<a href="#ttdownload" data-toggle="tab" class="producttab-heading">{l s='Download'}</a>
					</li>
				{/if}
				{if isset($product) && $product->customizable}
					<li class="">
					<a href="#ttpcustomization" data-toggle="tab" class="producttab-heading">{l s='Product customization'}</a>
					</li>
				{/if}
				{if isset($HOOK_PRODUCT_TAB_CONTENT) && $HOOK_PRODUCT_TAB_CONTENT}
					<li class="">
					<a href="#tttabcontent" data-toggle="tab" class="producttab-heading">{$HOOK_PRODUCT_TAB}</a>
					</li>
				{/if}
			</ul>
			<div class="tab-content">
				...
				{if isset($attachments) && $attachments}
					<ul id="ttdownload" class="tt-productinner tab-pane">
						<li>
				...
				{if isset($product) && $product->customizable}
					<ul id="ttpcustomization" class="tt-productinner tab-pane">
						<li>
				...
				{if isset($HOOK_PRODUCT_TAB_CONTENT) && $HOOK_PRODUCT_TAB_CONTENT}
					<ul id="tttabcontent" class="tt-productinner tab-pane ">
						<li>
							<div class="rte">{if isset($HOOK_PRODUCT_TAB_CONTENT) && $HOOK_PRODUCT_TAB_CONTENT}{$HOOK_PRODUCT_TAB_CONTENT}{/if}</div>
						</li>
					</ul>
				{/if}
			</div>
		...
		{if isset($accessories) && $accessories}
			...
			<section class="page-product-box accessories-product hb-animate-element bottom-to-top">
				<h4 class="title_block"><span class="ttblock-heading">{l s='Accessories'}</span></h4>
				<div class="block products_block accessories-block clearfix">
						...
						<ul id="ttaccessories-carousel" class="tt-carousel product_list grid clearfix">
							{foreach from=$accessories item=accessory name=accessories_list}
								{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) && $accessory.available_for_order && !isset($restricted_country_mode)}
									...
									<li class="item ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if}">
										<div class="product-container">
											<div class="left-block">
												<div class="product-image-container">
													<a href="{$accessoryLink|escape:'html':'UTF-8'}" title="{$accessory.legend|escape:'html':'UTF-8'}" class="product-image product_image">												
														  
															<img class="replace-2x img-responsive" src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')|escape:'html':'UTF-8'}" alt="{if !empty($accessory.legend)}{$accessory.legend|escape:'html':'UTF-8'}{else}{$accessory.name|escape:'html':'UTF-8'}{/if}" title="{if !empty($accessory.legend)}{$accessory.legend|escape:'html':'UTF-8'}{else}{$accessory.name|escape:'html':'UTF-8'}{/if}" {if isset($homeSize)} width="{$homeSize.width}" height="{$homeSize.height}"{/if} itemprop="image" />
															{hook h='productImageHover' id_product=$accessory.id_product}
												...
												<div class="ttquickview">
													{if isset($quick_view) && $quick_view}
														<a class="quick-view" href="{$accessory.link|escape:'html':'UTF-8'}" title="{l s='Quick view'}" rel="{$accessory.link|escape:'html':'UTF-8'}">
					...
					{if $tt_total > 4}
						<div class="customNavigation">
							<a class="btn prev ttaccessories_prev">{l s='Prev'}</a>
							<a class="btn next ttaccessories_next">{l s='Next'}</a>
						</div>
					{/if}
				</div>
			</section>
